var express = require('express')

var router = express.Router()
const bodyParser = require('body-parser');
var mysql = require('mysql');
var connection = require('express-myconnection');
const { v4: uuidv4 } = require('uuid');
const ResultSet = require('mysql/lib/protocol/ResultSet');
var nodemailer = require('nodemailer');
const xoauth2 = require('xoauth2');
var emailjs = require('@emailjs/browser');

router.get('/hello', function (req, res) {
  res.json('hello world123!')
})

router.get('/countries', function (req, res) {

  let con = mysql.createConnection({

    host: 'localhost', //'localhost',
    user: 'root',
    password: 'Amirejaz2412',
    port: 3306, //port mysql
    database: 'landingpage',
    queryTimeout: 6000,
    connectTimeout: 60000
  });
  con.connect(function (err) {

    con.query("SELECT code, name FROM country", function (error, results, fields) {
      if (error) { res.json(""); }
      else res.json(results);
    });
  });
})

router.post('/userExists', function (req, res) {

  let con = mysql.createConnection({

    host: 'localhost', //'localhost',
    user: 'root',
    password: 'Amirejaz2412',
    port: 3306, //port mysql
    database: 'landingpage',
    queryTimeout: 6000,
    connectTimeout: 60000
  });
  var table = req.isBusiness ? "business" : "user";
  con.connect(function (err) {

    con.query("SELECT * FROM" + table +"where email = ?", req.body.email, function (error, results, fields) {
      if (error) { res.json(""); }
      else if (results.length > 0) res.json(results[0].email);
      else res.json(false);
    });
  });
})


router.post('/add-user', function (req, res) {
  let con = mysql.createConnection({

    host: 'localhost', //'localhost',
    user: 'root',
    password: 'Amirejaz2412',
    port: 3306, //port mysql
    database: 'landingpage',
    queryTimeout: 6000,
    connectTimeout: 60000

  }); //or single
  var uuid = uuidv4();
  var datetime = new Date();
  var email = req.body.email;
  var country = req.body.country;
  var postalcode = req.body.postalcode;
  var communityId = -1;
  var referralCode = uuidv4();
  var referredByUser = req.referredByUser == undefined ? null : req.referredByUser;
  var referredByBusiness = req.referredByBusiness == undefined ? null : req.referredByBusiness;
  var isActive = 1;
  con.connect(function (err) {
    if (err) { throw err; }
    console.log("Connected!");
    const result = con.query("INSERT INTO community (postal_code, country_id, is_active) VALUES ?", [[[postalcode, "1", "1"]]], async function (error, results, fields) {
      if (error)
        throw error;
      communityId = results.insertId;
      con.query("INSERT INTO user (uuid, created_at, email, community_id, referral_id, referred_by, referred_by_user, is_active) VALUES ?", [[[uuid, datetime, email, communityId, referralCode, referredByBusiness, referredByUser, isActive]]], function (error, results, fields) {
        if (error) { res.json(""); }
        else res.json(referralCode);
      });
    });
  });
})
router.post('/add-business', function (req, res) {
  let con = mysql.createConnection({

    host: 'localhost', //'localhost',
    user: 'root',
    password: 'Amirejaz2412',
    port: 3306, //port mysql
    database: 'landingpage',
    queryTimeout: 6000,
    connectTimeout: 60000

  }); //or single
  var uuid = uuidv4();
  var datetime = new Date();
  var email = req.body.email;
  var country = req.body.country;
  var postalcode = req.body.postalcode;
  var communityId = -1;
  var referralCode = uuidv4();
  var referredByUser = req.referredByUser == undefined ? null : req.referredByUser;
  var referredByBusiness = req.referredByBusiness == undefined ? null : req.referredByBusiness;
  var isActive = 1;
  con.connect(function (err) {
    if (err) { throw err; }
    const result = con.query("INSERT INTO community (postal_code, country_id, is_active) VALUES ?", [[[postalcode, "1", "1"]]], async function (error, results, fields) {
      if (error)
        throw error;
      communityId = results.insertId;
      con.query("INSERT INTO business (uuid, created_at, email, community_id, referral_id, referred_by, referred_by_business, is_active) VALUES ?", [[[uuid, datetime, email, communityId, referralCode, referredByBusiness, referredByUser, isActive]]], function (error, results, fields) {
        if (error) { res.json(""); }
        else res.json(referralCode);
      });
    });
  });
})


router.post(("/send-email"), async (req, res) => {
  const frommail = req.body.fromemail
  const tomail = req.body.toemail
  vcode = Math.floor(100000 + Math.random() * 900000);
  var data = {
    "verification_code": vcode,
    "to_email": tomail
  }
  emailjs.send('landingpage_1234', 'template_gons7wu', data, 'user_CrBoZZHqq9leyv1IiNhWs')
    .then((result) => {
      console.log(result.text);
    }, (error) => {
      console.log(error.text);
    });



  const password = "FizzyBoy2412"
  // var transporter = nodemailer.createTransport({
  //     host: "smtp.gmail.com",
  //     port: 578,
  //     secure: true, // use TLS
  //     auth: {
  //       user: "mamirejaz75@gmail.com",
  //       pass: "FizzyBoy2412",
  //     },
  //     tls: {
  //       // do not fail on invalid certs
  //       rejectUnauthorized: false,
  //     },
  //   });

  var transporter = nodemailer.createTransport({
    service: 'Gmail',
    auth: {
      xoauth2: {
        type: 'OAuth2',
        user: 'mamirejaz75@gmail.com',
        clientId: "157756659296-bjehclu9k68lmf7c5sj5gonmlde8t5o8.apps.googleusercontent.com",
        clientSecret: "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",
        refreshToken: "1//04NNGq72EDxeeCgYIARAAGAQSNwF-L9IrbSQ-CemX_2-6AP1T1cSwP1pcJcBOrMNFUXO2CRDVIlXuabf3kldB57NGSQ2iedDnLEY",
        accessToken: 'ya29.A0ARrdaM8vho0_y27hJ3mQ3FisbMv89KfzIoA7klPw9_qFYUoNOGbd_i4LJ9uKZ0Z6WfvxYZoycVimoj_zBWdiA1yGE275TP42d7kwuRbAPBnCLUpJB1QdU8AViZYqIsVFnL4LPk1bMcFwcntIrnVCZzwEbCWK'
      }
    },
    tls: {
      rejectUnauthorized: false
    },
    port: 578,
    timeout: 8000

  });

  var mailOptions = {
    from: frommail,
    to: tomail,
    subject: 'Sending Email using Node.js',
    text: `sending mail using Node.js was running successfully. Hope it help you. For more code and project Please Refer my github page`
    // html: '<h1>Hi Smartherd</h1><p>Your Messsage</p>'        
  };
  transporter.verify((error, success) => {
    if (error) {
      console.log(error);
    } else {
      console.log('All works fine, congratz!');
    }
  });
  transporter.sendMail(mailOptions, function (error, info) {
    if (error) {
      res.json(false);
    }
    else {
      res.json(true)
    }
  });

})

// router.post('/send-email', function (req, res) {
//     let con = mysql.createConnection({

//         host: 'localhost', //'localhost',
//         user: 'root',
//         password: 'Amirejaz2412',
//         port: 3306, //port mysql
//         database: 'landingpage',
//         queryTimeout: 6000,
//         connectTimeout: 60000

//     }); //or single
//     var uuid = uuidv4();
//     var datetime = new Date();
//     var email = req.body.email;
//     var country = req.body.country;
//     var postalcode = req.body.postalcode;
//     var communityId = -1;
//     var referralCode = uuidv4();
//     var referredByUser = req.referredByUser == undefined ? null : req.referredByUser;
//     var referredByBusiness = req.referredByBusiness == undefined ? null : req.referredByBusiness;
//     var isActive = 1;
//     con.connect(function (err) {
//         if (err) { throw err; }
//         console.log("Connected!");
//         const result = con.query("INSERT INTO community (postal_code, country_id, is_active) VALUES ?", [[[postalcode, '1', '1']]], async function (error, results, fields) {
//             if (error)
//                 throw error;
//             communityId = results.insertId;
//             con.query("INSERT INTO business (uuid, created_at, email, community_id, referral_id, referred_by, referred_by_user, is_active) VALUES ?", [[[uuid, datetime, email, communityId, referralCode, referredByBusiness, referredByUser, isActive]]], function (error, results, fields) {
//                 if (error) { res.json(""); }
//                 else res.json(referralCode);
//             });
//         });
//     });
// })



module.exports = router;